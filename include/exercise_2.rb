class Tuxlang
  ...
  def ex_2_check(word,clicked)
    if word.name == clicked
        ...
        ex_2_main(@exercising_now[@exercise[0]],:good) if !@exercise.empty?
    elsif word.synonyms.include?(clicked) && !clicked.empty?
        @exercise.delete_at(0)
        ex_2_main(@exercising_now[@exercise[0]],:close,word.name) if !@exercise.empty?
    else
        @exercise << @exercise.delete_at(0)
        ex_2_main(@exercising_now[@exercise[0]],:bad,word.name) if !@exercise.empty?
    end
    
    next_exercise if @exercise.empty? and @learning_now
    repetition_end if @exercise.empty? and @repetition_now
  end
