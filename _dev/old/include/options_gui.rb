class Tuxlang
  def gui_opt_about_button(redirection)
    @gui[:opt_about_button]=Gtk::Button.new("About").set_size_request(200,50)
    @gui[:opt_about_button].signal_connect("clicked") { eval redirection }
    @gui[:content].put(@gui[:opt_about_button],10,10)
  end
  
  def gui_opt_keybinds_button(redirection)
    @gui[:opt_keybinds_button]=Gtk::Button.new("Keybindings").set_size_request(200,50)
    @gui[:opt_keybinds_button].signal_connect("clicked") { eval redirection }
  ...
  def gui_opt_awesome_button
    ...
    @gui[:opt_awesome_button].signal_connect("clicked") { eval redirection }
